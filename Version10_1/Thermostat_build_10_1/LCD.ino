...
void read_button()
{
  ...
  switch(localKey)
  {
    ...
    {
        if(system_status == 2) //system status BEFORE key pressed
          system_status = 0;
        else
          system_status++;
        ...
        if(system_status == 2) //if auto is set for system then auto is set for fan
          fan_status = 2;
    ...
    case 2: //FAN KEY 
    {
        if(system_status == 0) //if system status is off then fan is also off
            fan_status = 0;
        if(system_status == 2) //if auto is set for system then fan is set to auto mode
            fan_status = 2;
...
void set_display()
{
  ...
  {
    case 0:  //DISPLAY "OFF"
          strcpy_P(buffer, (char*)pgm_read_word(&off_));
          lcd.print(buffer);
          break;
    case 1:  //DISPLAY "MAN"
          lcd.print(F("MAN"));
          break;
    case 2:  //DISPLAY "AUT" 
          strcpy_P(buffer, (char*)pgm_read_word(&auto_));
          lcd.print(buffer);
          break;
  }
  lcd.setCursor(9, 0);
