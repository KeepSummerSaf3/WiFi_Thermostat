 ...
 *  It is also probably best noted here that this uses a modified Duemillanove board from Linksprite.
	...
	3. Allows for communication with NOAA METARs to help improve system efficiency. 
           Data retrieved includes outside temperature and outside barometric pressure from closest airfield.
           METARs are retrieved, decoded and formatted on my serve to significantly reduce overhead and complication.
 ...
 *						  professional right now either.
 ...
 Version changes from v4.2 to v4.3
 1. Added AC/HEAT ON/OFF display to 2nd line of LCD while in Manual Mode
 2. Shortened the sensor retrieval time to 90 seconds
...
void setup()
{
  ...
  pinMode(11, OUTPUT); // HEAT PUMP RELAY PIN
...
void loop()
{
  ...
  wireless_loop();
  sensor_loop();
  if(five_min_cnt > 254) //approx. 4 minute 15 sec timer  (255 seconds)
  {
    smart_functions();
    five_min_cnt = 0;
  }
