...
const prog_char ssid[] PROGMEM = {"KONGSBERG Guest"};	//max 32 bytes for SSID
  ...
  if(len != -1)
  {
    if(pass == 0)
    {
       tmp = strstr(data, "\r\n\r\n"); 
		...
		/*
		if(string == "Date: ")
		setTime("fields for hour", "fields for minute", "fields for sec", "fields for day", "fields for month", "fields for year")
		string DOW = "fields for day of week";
...
boolean sendPage(char* URL) {
  ...
  else if(strcmp(URL, "/config") == 0)
  {
    ...
    WiServer.print(local_ip[0]);
    WiServer.print(',');
    WiServer.print(local_ip[1]);
    WiServer.print(',');
    WiServer.print(local_ip[2]);
    WiServer.print(',');
    WiServer.print(local_ip[3]);
    ...
    //POST THE FORM AND THEN HAVE IT SENT BACK TO THE ARDUINO FOR PROCESSING
//     <form>
//      Temp Deadband: <input type="text" name="deadband"> F<br>
//      Heater Cutoff Temp: <input type="text" name="heat_cutoff"> F<br>
//      AC Cutoff Temp: <input type="text" name="ac_cutoff"> F<br>
//      Humidity Cutoff: <input type="text" name="humid_cutoff"> %<br>
//      Vacancy Temp Control: <input type="radio" name="vacant" value="Yes"> <br>
//     </form> 
...
void wireless_loop(){
  ...
  if(((minute() % 30) == 0) && !(METAR_req.isActive()))  //METAR request every 60 minutes
  {
      //pass = 0;
