...
void smart_functions()
{
  if(system_status == 3)
  {
    if(!vacant)
    {
      if((metar_table[0].temperature > 80 && indoor_temp >= 80) || indoor_humidity > 65) //indoor temperature kept at 80 if outside is above 80 
      {
  ...
  {
    //AC ON
     if(request_temperature <= indoor_temp && indoor_temp > 65 && temp_diff > 2)
     {
       AC = true;
       HEAT = false;
     }
     //AC OFF
     else if(request_temperature >= indoor_temp && indoor_temp > 65 && temp_diff > 2)
     {
         AC = false;
         HEAT = false;    
     }
     //HEAT ON
     else if(request_temperature >= indoor_temp && indoor_temp < 60 && temp_diff > 2)
     {
         AC = false;
         HEAT = true;
     }
