...
//unsigned char local_ip[] = {192,168,1,10};
uint8 local_ip[] = {192,168,1,10};     // IP address of WiShield
...
const prog_char ssid[] PROGMEM = {"OConnors"};	//max 32 bytes for SSID

unsigned char security_type = 3;	// 0 - open; 1 - WEP; 2 - WPA; 3 - WPA2

// WPA/WPA2 passphrase
const prog_char security_passphrase[] PROGMEM = {"soccer12"};	// max 64 characters password
...
void METARServerData(char * data, int len) 
{
  ...
  if(len != -1)
  {
    if(pass == 0)
    {
       tmp = strstr(data, (const char*)F("\r\n\r\n")); 
	   
       if(strstr(data, (const char*)F("Date: ")))
       {
    ...
    {
       tmp = strstr(data, (const char*)F("temperature:"));
       if(tmp != NULL)
       {
         if(tmp[13] == '-')
         {
       ...
       tmp = strstr(data, (const char*)F("pressure:"));
...
void wireless_loop(){
  ...
  if(((minute() % 30) == 0) && (second() == 0) && !(METAR_req.isActive()))  //METAR request every 60 minutes
  {
      //pass = 0;
